...
int main() {
	...
	MotionGenerator motionGenerator("./data", "./00_handstand.json");
	...
	Spec::BPSurvivorSelection::Parametrisable combinedSurvivorSelectorLambda = [=](DEvA::ParameterMap parameters, Spec::IndividualPtrs iptrs) {
		...
		DEvA::ParameterMap parameterMap({ {"metrics", j}, {"clusterMetric", "angularVelocitySign"} });
		iptrs = MGEA::cullPartiallyDominated(parameterMap, iptrs);
